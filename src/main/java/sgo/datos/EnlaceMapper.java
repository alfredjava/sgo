...
public class EnlaceMapper implements RowMapper<Enlace>{
	public Enlace mapRow(ResultSet rs, int arg1) throws SQLException 
	{
		...
		try {
			...
			eEnlace.setTitulo(rs.getString("titulo"));
			eEnlace.setUrlCompleta(rs.getString("url_completa"));
			eEnlace.setUrlRelativa(rs.getString("url_relativa"));
			...
			ePermiso.setNombre(rs.getString("desc_permiso"));
			...
			eEnlace.setUsuarioCreacion(rs.getString("usuario_creacion"));
		} catch(Exception ex){
			ex.printStackTrace();
		}
		return eEnlace;
