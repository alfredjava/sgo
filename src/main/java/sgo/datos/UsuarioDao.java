...
public class UsuarioDao {
	...
	public RespuestaCompuesta guardarRegistro(Usuario usuario){
		...
		try {
			...
			consultaSQL.append(" (nombre,clave,identidad,zona_horaria,estado,email,cambio_clave,rol,creado_el,creado_por,actualizado_por,actualizado_el,ip_creacion,ip_actualizacion,tipo,operacion) ");
			consultaSQL.append(" VALUES (:Nombre,:Clave,:Identidad,:ZonaHoraria,:Estado,:Email,:CambioClave,:Rol,:CreadoEl,:CreadoPor,:ActualizadoPor,:ActualizadoEl,:IpCreacion,:IpActualizacion,:Tipo,:Operacion) ");
...
//			listaParametros.addValue("IdRol", usuario.getIdRol());
	...
	public RespuestaCompuesta actualizarRegistro(Usuario usuario){
		...
		try {
			...
			consultaSQL.append("operacion=:Operacion,");
			...
			listaParametros.addValue("Rol", usuario.getId_rol());
			listaParametros.addValue("Tipo", usuario.getTipo());
			listaParametros.addValue("Operacion", usuario.getOperacion());
			//Valores Auditoria
			listaParametros.addValue("ActualizadoEl", usuario.getActualizadoEl());
			listaParametros.addValue("ActualizadoPor", usuario.getActualizadoPor());
			listaParametros.addValue("IpActualizacion", usuario.getIpActualizacion());
	...
	public Respuesta createRecord(Usuario usuario){		
		...
		try {			
			...
			sbSQL.append(" (nombre,clave,zona_horaria,habilitado,rol,nombre_real,apellido_real,direccion,email,cambio_clave,cmp,imagen) ");
			sbSQL.append(" VALUES (:nombre,sha1(:clave),:zonaHoraria,:habilitado,:rol,:nombreReal,:apellidoReal,:direccion,:email,:cambioClave,:cmp,:imagen)");
	...
	public Usuario getRecord (String username){
		...
		sqlSB.append("SELECT u.id_usuario,u.nombre, u.clave,");
		sqlSB.append("u.identidad, u.zona_horaria,u.estado,u.rol as id_rol,u.email,u.tipo,r.nombre as nombre_rol, u.id_operacion as id_operacion,u.id_cliente, k.referencia_planta_recepcion, k.referencia_destinatario_mercaderia, k.volumen_promedio_cisterna, k.nombre as nombre_operacion");
        sqlSB.append(" FROM ");
        sqlSB.append(NOMBRE_TABLA);
        sqlSB.append(" u INNER JOIN ");
        sqlSB.append(TABLA_ROL);
        sqlSB.append(" r ON u.rol = r.id_rol ");
        sqlSB.append(" INNER JOIN sgo.operacion k ON u.id_operacion = k.id_operacion ");
        sqlSB.append(" WHERE u.nombre=?");
        Object[] params = {username};
        try{
            mUsuario=(Usuario) jdbcTemplate.queryForObject(sqlSB.toString(), new UsuarioMapper(),params);
            sqlSB.setLength(0);
            sqlSB.append("SELECT pr.id_prol, pr.id_permiso, p.estado, p.nombre AS nombre,p.creado_por,p.creado_el,p.actualizado_por,p.actualizado_el,p.usuario_actualizacion,p.usuario_creacion,p.ip_actualizacion,p.ip_creacion");
            sqlSB.append(" FROM ");
