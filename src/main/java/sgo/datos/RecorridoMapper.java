...
public class RecorridoMapper implements RowMapper<Recorrido>{
	public Recorrido mapRow(ResultSet rs, int arg1) throws SQLException 
	{
		...
		try {
			...
			ePlantaOrigen.setDescripcion(rs.getString("descripcion_planta_origen"));
			eRecorrido.setPlantaOrigen(ePlantaOrigen);
			
			ePlantaDestino = new Planta();
			ePlantaDestino.setId(rs.getInt("planta_destino"));
			ePlantaDestino.setDescripcion(rs.getString("descripcion_planta_destino"));
			...
			eRecorrido.setUsuarioActualizacion(rs.getString("usuario_actualizacion"));
			eRecorrido.setUsuarioCreacion(rs.getString("usuario_creacion"));
