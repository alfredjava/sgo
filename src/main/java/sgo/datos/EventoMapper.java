...
public class EventoMapper implements RowMapper<Evento>{
	public Evento mapRow(ResultSet rs, int arg1) throws SQLException 
	{
		...
		try {
			...
			Date e = df2.parse(rs.getString("fecha_hora").toString());

			eEvento.setFechaHora(e);
			eEvento.setDescripcion(rs.getString("descripcion"));
			...
			eEvento.setUsuarioCreacion(rs.getString("usuario_creacion"));
			eEvento.setIpCreacion(rs.getString("ip_creacion"));
